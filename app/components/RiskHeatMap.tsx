  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <h3 className="text-sm font-semibold text-slate-700 uppercase tracking-wider">5×5 Risk Heat Map</h3>
        <InfoTooltip
      ...
      <div className="flex gap-3">
        <div className="flex items-center justify-center w-6">
          <p className="text-[10px] text-slate-400 uppercase tracking-widest -rotate-90 whitespace-nowrap">
            Likelihood →
          </p>
        </div>
        ...
        <div className="flex-1 space-y-1">
          ...
          {[5, 4, 3, 2, 1].map((lh) => (
            <div key={lh} className="flex gap-1 items-center">
              <span className="text-[10px] text-slate-400 w-4 text-right flex-shrink-0">{lh}</span>
              {[1, 2, 3, 4, 5].map((imp) => {
                const key = `${lh}-${imp}`;
                ...
                return (
                  ...
                  >
                    ...
                    {count > 0 && (
                      <span className="absolute top-1 right-1 w-4 h-4 rounded-full bg-white text-slate-700 border border-slate-200 text-[9px] font-bold flex items-center justify-center">
                        {count}
                      </span>
                    )}
                  </div>
          ...
          <p className="text-center text-[10px] text-slate-400 uppercase tracking-widest mt-0.5">
            ← Impact
          </p>
        </div>
      </div>
